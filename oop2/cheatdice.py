...
class Cheat_Loaded_Dice(Player): # inheritance of Player
 ...
 #custom class that allows 4 rolls cheat         
class Cheat_Multiroll(Player): #inherits Player
    def cheat(self):
        self.dice = []
        for i in range(4): #makes 4 rolls
            self.dice.append(randint(1,6))

#custom class forces other player lower rolls only
class Cheat_Other(Player):
    def cheat(self, cheater:Player):
        cheater.dice = [] # clears current dice
        for i in range(3):  # make 3 rolls
            cheater.dice.append(randint(1,2))   #rolls only 1 or 2 (low) 
